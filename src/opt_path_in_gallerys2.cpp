#include "opt_path_in_gallerys2.h"
...
void GenerateGallery::generateQpProblam() {
    ...
    vector<double> d;    //记录每条走廊需要的路径长度
    for(uint i = 0; i < gallerys_.size(); i++) {
        double d0;
        if(gallerys_.size() > 1) {
            if(i == 0) {
                d0 = hypot(0.5 * intersection_areas[i].y_min_ + 0.5 * intersection_areas[i].y_max_ - y0, 0.5 * intersection_areas[i].x_min_ + 0.5 * intersection_areas[i].x_max_ - x0);
            }
            else if(i == gallerys_.size() - 1) {
                d0 = hypot(0.5 * intersection_areas[i - 1].y_min_ + 0.5 * intersection_areas[i - 1].y_max_ - y1, 0.5 * intersection_areas[i - 1].x_min_ + 0.5 * intersection_areas[i - 1].x_max_ - x1);
            }
            else {
                d0 = hypot(0.5 * (intersection_areas[i].y_min_ + intersection_areas[i].y_max_ - intersection_areas[i - 1].y_min_ - intersection_areas[i - 1].y_max_), 
                           0.5 * (intersection_areas[i].x_min_ + intersection_areas[i].x_max_ - intersection_areas[i - 1].x_min_ - intersection_areas[i - 1].x_max_));
            }
        }
        else {
            d0 = hypot(y1 - y0, x1 - x0);
        }
        d.push_back(d0);
    }
